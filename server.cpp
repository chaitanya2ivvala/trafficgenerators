...
const char * version="0.600";



void print_version(){
  printf("Version: %s \n",VERSION);
  printf("%s / %s / %s / %s \n",program_name, version, build_git_time, build_git_sha);
}
 

void print_usage(){
      printf("-U || --udp                       UDP mode.\n");
      printf("-T || --tcp                       TCP mode.\n");
...
int main(int argc, char *argv[])
{
  ...
  int wildcard=1;
  int loglevel=0;
  enum daemontype {NONE,UDPd, TCPd, ICMPd};
  daemontype servertype=NONE;
  ...
  while ( (op =getopt_long(argc, argv, "UTIt:f:k:e:r:p:hdL:l:w",long_options, &option_index))!=EOF) {
    switch (op){
    case 'U': /* UDP server */
      servertype=UDPd;
      break;
  ...
  if(servertype==NONE){
    printf("You need to specify server type.\n");
    exit(EXIT_FAILURE);
  }
